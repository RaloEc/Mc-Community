    ...
    storage: isBrowser ? {
      getItem: (key) => {
        try {
          ...
          console.log(`[Auth] Recuperando clave ${key}: ${item ? 'Encontrada' : 'No encontrada'}`);
          return item;
        } catch (error) {
...
export const getServiceClient = () => {
  try {
    ...
    if (!serviceKey || serviceKey.length < 30) {
      console.warn('SUPABASE_SERVICE_KEY no válida, intentando con NEXT_PUBLIC_SUPABASE_SERVICE_KEY');
    ...
    if (!serviceKey || serviceKey.length < 30) {
      console.warn('No se encontró una clave de servicio válida, usando clave anónima (esto puede causar problemas con RLS)');
    ...
    console.log(`Creando cliente de servicio de Supabase con URL: ${supabaseUrl}`);
    ...
    serviceClientInstance = createClient(supabaseUrl, serviceKey, {
      ...
      global: {
        headers: {
          'x-application-name': 'mc-community-admin', // Identificador para depuración
        },
      },
    });
    ...
    if (!serviceClientInstance) {
      console.error('No se pudo crear el cliente de servicio, usando cliente normal como fallback');
      return supabase;
    }
