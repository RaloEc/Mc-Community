...
export async function POST(request: Request) {
  try {
    ...
    console.log('[API Comentarios Reply] Buscando comentario padre con ID:', parent_id);
    
    const { data: parentComment, error: parentError } = await serviceSupabase
      .from('comentarios')
      .select('id, tipo_entidad, entidad_id, contenido, usuario_id, comentario_padre_id')
      .eq('id', parent_id)
      .single();
      
    console.log('[API Comentarios Reply] Resultado de búsqueda del comentario padre en comentarios:', { 
    ...
    if (parentError || !parentComment) {
      ...
      if (!forumError && forumPost) {
        ...
        console.log('[API Comentarios Reply] Encontrado post del foro como padre:', forumPost.id);
      } else {
        console.error('[API Comentarios Reply] Error: El comentario padre no existe en ninguna tabla');
        return NextResponse.json(
    ...
    if (isForoPost) {
      ...
      if (parentForoError) {
        console.error('[API Comentarios Reply] Error al obtener post padre:', parentForoError);
        return NextResponse.json(
      ...
      if (parentComentarioError || !parentComentarioData) {
        console.error('[API Comentarios Reply] Error al obtener comentario padre:', parentComentarioError);
        return NextResponse.json(
          { success: false, error: 'No se pudo obtener el comentario padre. Detalles: ' + (parentComentarioError?.message || 'Comentario no encontrado') },
          { status: 500 }
        );
      }
    ...
    if (isForoPost) {
      ...
      const { data: replyPostData, error: replyPostError } = await serviceSupabase
        .from('foro_posts')
        .insert({
          contenido: text,
          autor_id: user.id,
      ...
      if (replyPostError) {
        console.error('[API Comentarios Reply] Error al crear respuesta en foro_posts:', replyPostError);
        return NextResponse.json(
          { success: false, error: `Error al crear la respuesta: ${replyPostError.message}` },
          { status: 500 }
        );
      }
      ...
      console.log('[API Comentarios Reply] Respuesta en foro_posts creada exitosamente:', respuestaFormateada.id);
      ...
      if (!parentComment) {
        console.error('[API Comentarios Reply] Error: parentComment es null o undefined');
        return NextResponse.json(
          { success: false, error: 'Error interno: datos del comentario padre no disponibles' },
          { status: 500 }
        );
      }
      ...
      if (parentComment.comentario_padre_id) {
        console.log('[API Comentarios Reply] El comentario padre es a su vez una respuesta, buscando comentario raíz');
        ...
        for (let i = 0; i < 10 && !foundRoot; i++) {
          ...
          if (rootError || !rootComment) {
            console.error('[API Comentarios Reply] Error al buscar comentario raíz:', rootError);
          ...
          if (!rootComment.comentario_padre_id) {
            ...
            foundRoot = true;
            console.log('[API Comentarios Reply] Encontrado comentario raíz:', rootComment.id);
          } else {
      ...
      console.log('[API Comentarios Reply] Usando tipo_entidad:', tipo_entidad, 'y entidad_id:', entidad_id);
      ...
      if (error) {
        console.error('[API Comentarios Reply] Error al crear respuesta:', error);
        return NextResponse.json(
          { success: false, error: `Error al crear la respuesta: ${error.message}` },
          { status: 500 }
        );
      }
      ...
      console.log('[API Comentarios Reply] Respuesta creada exitosamente:', respuestaFormateada.id);
