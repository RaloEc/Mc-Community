 ...
 * GET /api/riot/login
 *
 * Construye la URL de autorización de Riot OAuth 2.0 y redirige al usuario
 * a la página de login de Riot Games.
 *
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!clientId || !redirectUri) {
      console.error("[Riot OAuth] Variables de entorno faltantes:", {
        clientId: !!clientId,
    ...
    console.log("[Riot OAuth] Redirigiendo a:", authorizationUrl.toString());

    // Redirigir al usuario a la página de login de Riot
    return NextResponse.redirect(authorizationUrl.toString());
  } catch (error: any) {
    console.error("[Riot OAuth] Error en endpoint /login:", error);
