...
export async function GET(request: Request, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    try {
      ...
      console.warn('No se pudo incrementar las vistas del hilo (la función RPC puede no existir):', rpcError);
    }
    ...
    if (error) {
      ...
      return NextResponse.json({ error: 'Error al cargar el hilo: ' + error.message }, { status: 500 });
    }
    ...
    return NextResponse.json({ error: 'Error inesperado en el servidor: ' + error.message }, { status: 500 });
  }
}
