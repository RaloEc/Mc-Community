...
export async function GET(request: NextRequest) {
  ...
  console.log('Usuario es administrador. Usando cliente de servicio para operación administrativa')
  ...
  try {
    ...
    if (countError) {
      ...
      if (countError.code === '42P01') { // UNDEFINED_TABLE
        console.log('La tabla noticias_categorias no existe, continuando con la eliminación')
      } else {
        throw countError
      }
      ...
      if (!forzarEliminacion) {
        ...
        if (!noticiasError && noticiasAsociadas && noticiasAsociadas.length > 0) {
          ...
          if (titulos) {
            mensajeError += ` Ejemplos: ${titulos}${noticiasAsociadas.length < count ? '...' : ''}`;
          }
        }
        
        mensajeError += ' Puedes usar el parámetro "forzar=true" para mover automáticamente estas noticias a la categoría "General" antes de eliminar.';
        ...
        console.log(`Forzando eliminación de categoría ${id} y moviendo ${count} noticias a categoría General`);
        ...
        if (errorBusqueda && errorBusqueda.code !== 'PGRST116') { // Error diferente a "no encontrado"
          throw errorBusqueda;
        }
        ...
        if (categoriaGeneral) {
          ...
          console.log('Categoría General encontrada con ID:', categoriaGeneralId);
        } else {
          ...
          const { data: nuevaCategoria, error: errorCreacion } = await supabase
            ...
            .insert({
              nombre: 'General',
              slug: 'general',
              descripcion: 'Categoría general para noticias sin categoría específica',
        ...
        if (!asociaciones || asociaciones.length === 0) {
          ...
          const { data: asociacionesExistentes, error: errorExistentes } = await supabase
            .from('noticias_categorias')
            .select('noticia_id')
          ...
          const noticiasIdsExistentes = asociacionesExistentes?.map(a => a.noticia_id) || [];
          const noticiasAReasignar = noticiasIds.filter(id => !noticiasIdsExistentes.includes(id));
          
          console.log(`Reasignando ${noticiasAReasignar.length} noticias a la categoría General`);
          ...
          if (noticiasAReasignar.length > 0) {
            ...
            console.log(`${nuevasAsociaciones.length} noticias reasignadas correctamente`);
          }
          ...
          console.log(`Asociaciones con la categoría ${id} eliminadas correctamente`);
        }
      }
    }
    console.log(`Intentando eliminar categoría con ID: ${id} con cliente de servicio`)
    const { data, error } = await supabase
      .from('categorias')
      .delete()
      .eq('id', id)
    ...
    if (checkError && checkError.code === 'PGRST116') {
      ...
      console.log('Verificación exitosa: La categoría fue eliminada correctamente')
      return NextResponse.json({ success: true })
    } else {
      console.error('La categoría no fue eliminada. Todavía existe:', checkData)
      return NextResponse.json(
        { error: 'La categoría no fue eliminada aunque no se reportaron errores' },
        { status: 500 }
      )
    }
  } catch (error: any) {
