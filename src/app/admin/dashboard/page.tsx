...
function DashboardContent() {
  ...
  const fetchStats = useCallback(async (isRefresh = false) => {
    ...
    try {
      ...
      if (!supabase) {
        console.error('[Dashboard] No se pudo obtener el cliente Supabase para estadísticas');
      ...
      const [
        ...
        supabase.from('foro_hilos').select('*', { count: 'exact', head: true }),
    ...
    const loadRecent = async () => {
      ...
      try {
        ...
        const { data, error } = await supabase
          .from('noticias')
          .select('id, titulo, contenido, imagen_portada, fecha_publicacion, created_at, autor, autor_id')
        ...
        if (error) {
          ...
          if (ids.length > 0) {
            ...
            if (!perfilesError && Array.isArray(perfiles)) {
              ...
              for (const p of perfiles as { id: string, username?: string | null }[]) {
        ...
        const msg = e instanceof Error ? e.message : 'Error desconocido al cargar noticias'
        setRecentError(msg)
        setRecentNews([])
      } finally {
        setRecentLoading(false)
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-4">
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto">
          <Link href="/admin/noticias/crear" className="w-full sm:w-auto">
            <Button size="sm" className="w-full sm:w-auto dark:bg-zinc-900 dark:text-white dark:hover:bg-zinc-800">
            ...
            >
              ...
              <CardContent className="flex flex-col justify-center items-center py-3 px-3 md:px-4 flex-grow">
                <div className="text-3xl md:text-4xl font-bold tracking-tight text-center">
                  {statsLoading ? <Skeleton className="h-8 w-20 dark:bg-zinc-900 bg-zinc-200" /> : card.value}
                </div>
                {card.description && (
          ...
          <CardContent>
            <div className="flex items-center justify-center h-[200px]">
              <div className="text-center dark:text-zinc-400 text-zinc-500">
                ...
                <p className="dark:text-zinc-400 text-zinc-500">Próximamente: Gráficos de rendimiento</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <CardContent>
          {recentLoading ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
              {[1,2,3,4].map((i) => (
                <div key={i} className="flex items-center gap-3 p-3 rounded-lg border dark:border-zinc-800/60 border-zinc-200/80">
            ...
            <div className="text-sm text-muted-foreground amoled:text-gray-300">No hay noticias recientes.</div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
              {recentNews.map((n) => (
                <Link href={`/noticias/${n.id}`} key={n.id} className="group block" aria-label={`Abrir noticia: ${n.titulo || 'Sin título'}`}>
                  <div className="relative flex items-center gap-3 p-3 rounded-lg border border-transparent dark:border-zinc-800/60 amoled:border-zinc-800 hover:bg-zinc-50 dark:hover:bg-zinc-900/40 amoled:hover:bg-zinc-900/40 transition-colors cursor-pointer">
                    {n.imagen_portada ? (
                      <img src={n.imagen_portada} alt={n.titulo || 'Noticia'} className="h-10 w-10 rounded-md object-cover" />
                    ) : (
                      <div className="h-10 w-10 rounded-md bg-zinc-200 dark:bg-zinc-900" />
                    )}
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-gray-900 dark:text-white amoled:text-white truncate group-hover:underline">{n.titulo || 'Sin título'}</p>
                      <p className="text-xs text-zinc-600 dark:text-zinc-400 amoled:text-zinc-300 truncate">
                        {(n.fecha_publicacion || n.created_at) ? new Date(n.fecha_publicacion || n.created_at || '').toLocaleString('es-ES', { dateStyle: 'medium', timeStyle: 'short' }) : 'Fecha desconocida'}
                      </p>
                    </div>
                    <span className="absolute bottom-2 right-2 text-[10px] px-2 py-0.5 rounded-full bg-zinc-100 text-zinc-700 dark:bg-zinc-800/70 dark:text-zinc-300 amoled:bg-zinc-900/70 amoled:text-zinc-300">
                      {authorUsernames[(n.autor_id || n.autor || '')] || 'Autor desconocido'}
                    </span>
                  </div>
                </Link>
              ))}
