...
export default function EventosWidget({ className = "" }: EventosWidgetProps) {
  ...
  useEffect(() => {
    const fetchEventos = async () => {
      try {
        ...
        // Crear cliente de Supabase independiente (no depende del contexto)
  ...
  const getEventosDelDia = (dia: Date) => {
    return eventos.filter((evento) => isSameDay(new Date(evento.fecha), dia));
  };
  ...
  if (loading) {
    return (
      <div
        className={`rounded-xl p-4 border border-gray-200 dark:border-gray-800 ${className}`}
      >
    ...
    >
            ...
            eventosFuturos.map((evento) => (
              ...
              >
                <div className="flex-shrink-0">
                  <div className="relative w-14 h-14 rounded-lg bg-gray-100 dark:bg-gray-800 flex items-center justify-center overflow-hidden border border-gray-200 dark:border-gray-700">
                    <Image
                      src={
                        // Si es una URL completa, usarla directamente
                        evento.imagen_url?.startsWith("http") ||
                        evento.icono_url?.startsWith("http")
                      ...
                      sizes="56px"
                      onError={() => {
                        // Next.js Image maneja errores automáticamente con fallback
                      }}
                    />
                  </div>
                </div>
      ...
      {viewMode === "calendario" && (
        <div>
              ...
              return eventosDelDia.length > 0 ? (
                <TooltipProvider key={i}>
                  <Tooltip delayDuration={300}>
                    <TooltipTrigger asChild>{diaElement}</TooltipTrigger>
                    <TooltipContent
                      side="bottom"
                      align="center"
          ...
          <div className="mt-4">
            <h4 className="text-sm font-medium mb-2">Eventos de hoy</h4>
