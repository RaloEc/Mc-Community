...
import { ServerIcon, UsersIcon, TagIcon, ExternalLinkIcon, RefreshCw, CheckCircle, XCircle } from 'lucide-react'
...
export default function ServidorCard({ servidor, onRefresh }: ServidorCardProps) {
    ...
    <Card className={`servidor-card overflow-hidden transition-all hover:shadow-md dark:bg-amoled-gray ${status?.online ? 'online' : 'offline'}`}>
      ...
      <div className="servidor-info p-3">
        <div className="flex justify-between items-start mb-2">
          <div>
            <h1 className="server-name text-lg font-bold servidor-header-title">{servidor.nombre}</h1>
            <p className="server-desc text-xs text-muted-foreground line-clamp-2 servidor-header-description">{servidor.descripcion}</p>
          </div>
          <Badge variant="outline" className="flex items-center gap-1 h-6 servidor-header-badge">
        ...
        <div className="server-info grid grid-cols-2 gap-2 mb-3">
          <div className="info-item flex items-center gap-2">
            ...
            <div>
              <span className="label text-xs text-muted-foreground">Versión</span>
              <span className="value block text-xs font-medium">
                {status?.online ? status.version : servidor.version || 'Desconocida'}
              </span>
            </div>
          </div>
          ...
          <div className="info-item flex items-center gap-2">
            ...
            <div>
              <span className="label text-xs text-muted-foreground">Jugadores</span>
              <span className="value block text-xs font-medium">{formatPlayers()}</span>
            </div>
          </div>
        </div>
          ...
          <div className="motd p-2 rounded-md border border-border/50 bg-muted/30 dark:bg-amoled-gray/80 dark:border-border/30 mb-3">
            ...
            <div>
              {status.motd.clean.slice(0, 1).map((line, index) => (
                <p key={index} className="text-xs italic truncate">{line}</p>
              ))}
            </div>
          </div>
        )}
