...
export default function NoticiasLista({
  ...
  useEffect(() => {
    ...
    if (filtrosAplicados.busqueda) {
      activos.push({
        tipo: "busqueda",
        valor: filtrosAplicados.busqueda,
        etiqueta: "Búsqueda",
      });
    }
    ...
    if (filtrosAplicados.categoria) {
      activos.push({
        tipo: "categoria",
        valor: filtrosAplicados.categoria,
        etiqueta: "Categoría",
      });
    }
    if (filtrosAplicados.ordenFecha && filtrosAplicados.ordenFecha !== "desc") {
      activos.push({
        tipo: "ordenFecha",
        valor: filtrosAplicados.ordenFecha,
        etiqueta: "Orden",
      });
    }
  ...
  const handleInputChange = useCallback(
    (valor: string, tipo: "busqueda" | "autor" | "categoria") => {
      ...
      <div className={`grid ${gridCols[columnas]} gap-6 w-full`}>
        {noticiasConAnuncios.map((item: NoticiaConAnuncio) =>
          esAnuncio(item) ? (
            ...
            <div
              key={item.id}
              className="bg-card rounded-lg overflow-hidden shadow-md hover:shadow-lg transition-shadow duration-300 border border-border/50 relative dark:data-[theme=amoled]:bg-black"
            >
              <div className="relative h-48 overflow-hidden">
                ...
                <div className="flex flex-wrap gap-1 mb-2">
                  {item.categorias && item.categorias.length > 0 ? (
                    (() => {
                      ...
                      const categoriasPadre = item.categorias.filter(
                        (c) => !c.parent_id
                      );
                      ...
                      if (categoriasPadre.length > 0) {
                        ...
                        return (
                          <Badge
                            key={cat.id}
                            className={`text-xs ${
                              cat.color ? `border-${cat.color}-500 ` : ""
                            }`}
                            variant="outline"
                          >
                            {cat.icono && (
                        ...
                        if (categoriaPadre) {
                          return (
                            <Badge
                              ...
                              className={`text-xs ${
                                categoriaPadre.color
                                  ? `border-${categoriaPadre.color}-500 `
                                  : ""
                              }`}
                              variant="outline"
                            >
                          ...
                          return (
                            <Badge
                              ...
                              className={`text-xs ${
                                primerSubcategoria.color
                                  ? `border-${primerSubcategoria.color}-500 `
                                  : ""
                              }`}
                              variant="outline"
                            >
                              {primerSubcategoria.icono && (
                                <span className="mr-1">
                                  {primerSubcategoria.icono}
                                </span>
                              )}
                    ...
                    <Badge
                      className={`text-xs ${
                        item.categoria.color
                          ? `border-${item.categoria.color}-500 `
                          : ""
                      }`}
                      variant="outline"
                    >
                ...
                {item.imagen_url || (item as any).imagen_portada ? (
                  <Image
                    ...
                    onError={(e) => {
                      console.error(
                        "Error al cargar imagen de noticia en lista"
                      );
                      ...
                      fallback.className =
                        "text-primary-foreground text-lg font-medium absolute inset-0 flex items-center justify-center";
              ...
              {(item.autor_nombre || item.autor?.username) && (
                <span className="absolute bottom-2 left-4 text-xs text-muted-foreground">
                  <Link
                    ...
                    style={{
                      color: item.autor?.color || item.autor_color || "inherit",
                    }}
                  >
                    {item.autor?.username || item.autor_nombre}
                  </Link>
  ...
  return (
    <div className={`w-full flex flex-col items-center ${className}`}>
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <div className="flex-1">
                ...
                <input
                  ...
                  value={busquedaLocal}
                  onChange={(e) =>
                    handleInputChange(e.target.value, "busqueda")
                  }
                ...
                >
                  ...
                  {categorias.map((cat) => (
                    <option key={cat.id} value={cat.id}>
                      ...
                      {cat.parent_id ? "— " : ""}
                      {cat.nombre} {cat.icono ? cat.icono : ""}
                    </option>
                  ))}
                </select>
              </div>
      ...
      <div className="w-full flex justify-center mt-4">
        <div className="w-full">{renderContenidoNoticias()}</div>
      </div>
    </div>
  );
}
