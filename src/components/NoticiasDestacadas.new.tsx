...
// Tipo para categorías jerárquicas
            ...
            <div className="relative h-[350px] w-full">
              ...
              <div className="absolute top-0 left-0 p-3">
                <div className="flex flex-wrap gap-1 mb-2">
                  {noticiaPrincipal1.categorias && noticiaPrincipal1.categorias.length > 0 ? (
                    (() => {
                      // Buscar categorías padres (sin parent_id)
                      const categoriasPadre = noticiaPrincipal1.categorias.filter(c => !c.parent_id);
                      ...
                      // Si no hay categorías padre, buscar la primera subcategoría y su padre
                        ...
                        const primerSubcategoria = noticiaPrincipal1.categorias[0];
                        const categoriaPadre = primerSubcategoria.parent_id ? 
                          noticiaPrincipal1.categorias.find(c => c.id === primerSubcategoria.parent_id) : null;
                    ...
                    <Badge 
                      className={`text-xs ${noticiaPrincipal1.categoria.color ? `border-${noticiaPrincipal1.categoria.color}-500 ` : ''}`}
                      variant="outline"
                    >
                      {noticiaPrincipal1.categoria.icono && <span className="mr-1">{noticiaPrincipal1.categoria.icono}</span>}
                  ...
                  <div className="flex flex-wrap gap-2">
                    {noticiaPrincipal2.categorias ? (
                      noticiaPrincipal2.categorias.map(cat => {
                        // Determinar si la categoría tiene padre para mostrar la jerarquía
                        const categoriaParent = cat.parent_id ? 
                          noticiaPrincipal2.categorias.find(c => c.id === cat.parent_id) : null;
                        
                        return (
                          <Badge 
                            key={cat.id} 
                            variant="secondary" 
                            className={`${cat.color ? `border-${cat.color}-500 ` : ''}bg-primary/90 text-white shadow-md`}
                          >
                            {categoriaParent ? `${categoriaParent.nombre} > ` : ''}
                            {cat.icono && <span className="mr-1">{cat.icono}</span>}
                            {cat.nombre}
                          </Badge>
                        );
                      ...
                      <Badge 
                        variant="secondary" 
                        className={`${noticiaPrincipal2.categoria && noticiaPrincipal2.categoria.color ? `border-${noticiaPrincipal2.categoria.color}-500 ` : ''}bg-primary/90 text-white shadow-md`}
                      >
                        {noticiaPrincipal2.categoria && noticiaPrincipal2.categoria.icono && 
                          <span className="mr-1">{noticiaPrincipal2.categoria.icono}</span>}
                        {noticiaPrincipal2.categoria && noticiaPrincipal2.categoria.nombre}
                      </Badge>
                    ) : null}
                  </div>
                </div>
              ...
              <div className="relative h-32">
                ...
                <div className="absolute top-2 left-2">
                  {noticia.categorias && noticia.categorias[0] ? (
                    <Badge 
                      className={`${noticia.categorias[0]?.color ? `border-${noticia.categorias[0].color}-500 ` : ''}bg-primary/80 text-white text-xs`}
                    >
                      {noticia.categorias[0].parent_id ? (
                        <>
                          {noticia.categorias.find(c => c.id === noticia.categorias[0].parent_id)?.nombre} {' > '}
                        </>
                      ) : null}
                      {noticia.categorias[0]?.icono && <span className="mr-1">{noticia.categorias[0].icono}</span>}
                      {noticia.categorias[0]?.nombre}
                    </Badge>
                  ) : noticia.categoria ? (
                    <Badge 
                      className={`${noticia.categoria && noticia.categoria.color ? `border-${noticia.categoria.color}-500 ` : ''}bg-primary/80 text-white text-xs`}
                    >
                      {noticia.categoria && noticia.categoria.icono && <span className="mr-1">{noticia.categoria.icono}</span>}
                      {noticia.categoria && noticia.categoria.nombre}
                    </Badge>
                  ) : (
