...
const HtmlContentWithYoutube = ({ html, className = '' }: { html: string; className?: string }) => {
  ...
  const getContentWithoutMedias = (content: string, removeImages: boolean = false) => {
    if (!content) return '';
    const tempDiv = document.createElement('div');
    tempDiv.innerHTML = content;
  ...
  const contentWithoutMedias = getContentWithoutMedias(html, shouldRemoveImages);
  
  // Verificar si hay contenido de texto después de eliminar medios
  const hasTextContent = contentWithoutMedias.replace(/<[^>]*>/g, '').trim().length > 0;
  ...
  return (
    <div className={className}>
      ...
      {!containsVideo && !containsMultipleImages && images.length === 1 && (
        <div className="mb-3 cursor-zoom-in" onClick={() => setShowLightbox(true)}>
          <img 
            src={images[0]} 
            alt="Imagen" 
...
function HiloCard(props: HiloCardProps) {
  ...
  const handleCardClick = (e: React.MouseEvent) => {
    ...
    if (target.closest('iframe') || target.closest('.youtube-embed-container')) {
      e.preventDefault();
      e.stopPropagation();
      return;
    }
    ...
    <div className={`${className} block my-3`}>
      <Card 
        ...
        style={{
          borderWidth: '1px',
          borderStyle: 'solid',
          borderColor: userColor ? `${userColor}40` : 'rgba(156, 163, 175, 0.5)',
...
export default React.memo(HiloCard, (prevProps, nextProps) => {
