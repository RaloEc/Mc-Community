...
export default function HilosLista({ hilos, loading }: { hilos: HiloDTO[]; loading: boolean }) {
    ...
    return (
      <div className="space-y-3">
        {Array.from({ length: 4 }).map((_, i) => (
          <div key={i} className="h-20 rounded-lg border bg-muted dark:bg-black animate-pulse" />
        ))}
      </div>
    )
  }
  if (!hilos || hilos.length === 0) {
    return (
      <div className="rounded-lg border bg-card dark:bg-black p-6 text-center text-muted-foreground">
