      ...
      try {
        ...
        const timeoutPromise = new Promise<Response>((_, reject) => {
          timeoutId = setTimeout(() => {
            reject(
              new Error("Tiempo de espera agotado al cargar las noticias")
            );
          }, 10000); // 10 segundos
        });
        ...
        if (isMounted) {
          setError(
            err instanceof Error ? err.message : "Error al cargar las noticias"
          );
          setNoticias([]);
        }
      } finally {
          ...
          <div className="overflow-hidden" ref={emblaRef}>
            <div className="flex -ml-4">
              {Array.from({ length: featured ? 3 : limit }).map((_, i) => (
                <div
                  key={i}
                  className="flex-none w-full px-4 sm:w-1/2 md:w-1/3 lg:w-1/4"
                >
