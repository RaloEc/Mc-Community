...
export default function BotonReportarNoticia({
  tipo_contenido,
  contenido_id,
  variant = "ghost",
  size = "sm",
  hideLabelBelow,
}: BotonReportarNoticiaProps) {
  ...
  const handleReportar = async () => {
    ...
    try {
      ...
      setRazon("");
      setDescripcion("");
      // Invalidar la query de reportes para que se actualice en la página de admin
      queryClient.invalidateQueries({ queryKey: ["reportes-noticias"] });
    } catch (error) {
  ...
  const hideLabelBreakpoint = hideLabelBelow
    ? breakpointMap[hideLabelBelow]
    : null;
  const labelClassName = hideLabelBreakpoint
    ? `hidden ${hideLabelBreakpoint}:inline-flex`
    : "inline-flex";
  const hideLabelCompletely = size === "icon";

  return (
    <Dialog open={dialogAbierto} onOpenChange={setDialogAbierto}>
      <DialogTrigger asChild>
        <Button variant={variant} size={size} aria-label="Reportar">
          <Flag className={`h-4 w-4 ${hideLabelCompletely ? "" : "mr-2"}`} />
          {!hideLabelCompletely && (
            <span className={labelClassName}>Reportar</span>
          )}
        </Button>
      </DialogTrigger>
      <DialogContent>
