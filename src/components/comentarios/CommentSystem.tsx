...
const mapApiCommentToUI = (apiComment: any): Comment => {
  return {
    ...
    gif_url: apiComment.gif_url || null,
...
export const CommentSystem: React.FC<CommentSystemProps> = ({
  ...
  const loadComments = useCallback(
    async (reset = false) => {
      try {
        ...
        const url = `/api/comentarios?contentType=${encodeURIComponent(
          contentType
        )}&contentId=${encodeURIComponent(
          contentId
        )}&limite=${pageSize}&offset=${currentOffset}&orden=${order}`;
    ...
    [contentType, contentId, pageSize, order, offset]
  );

  // Cargar comentarios al montar el componente
  useEffect(() => {
    loadComments(true);
  }, [contentType, contentId, order, loadComments]);

  // Escuchar cambios de Realtime en comentarios
  useEffect(() => {
    const supabase = createClient();

    const channel = supabase
      .channel(`comentarios:${contentType}:${contentId}`)
      .on(
        "postgres_changes",
        {
          event: "INSERT",
        ...
        (payload) => {
          console.log("[CommentSystem] Cambio de Realtime detectado:", payload);
          // Recargar comentarios cuando hay un INSERT
          loadComments(true);
        }
      )
      .subscribe();
  ...
  const handleAddReply = async (
    ...
    try {
      ...
      // Usar rutas relativas
      console.log(
        "[CommentSystem] Enviando respuesta a:",
        "/api/comentarios/reply"
      );
