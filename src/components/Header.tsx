...
export default function Header() {
  ...
  return (
    <header className="bg-white/95 dark:bg-black/95 backdrop-blur-md fixed top-0 left-0 right-0 z-50 border-b border-gray-200/50 dark:border-gray-800/50 text-gray-900 dark:text-white shadow-sm w-full">
            ...
            <form onSubmit={handleSearch} className="relative">
              ...
              <Input
                ...
                style={{
                  ...
                  '--focus-ring-color': profile?.color ? `${profile.color}40` : 'rgba(59, 130, 246, 0.25)'
                } as React.CSSProperties}
      ...
      {/* Overlay semitransparente - comienza debajo del header - Mostrar en pantallas menores a 1024px */}
      <div 
        className={`lg:hidden fixed left-0 right-0 z-[998] transition-all duration-300 ${isMenuOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
      ...
      {/* Menú desplegable con animación desde el botón - visible en pantallas menores a 1024px */}
      <div 
        className={`lg:hidden fixed top-16 right-4 z-[999] ${isMenuOpen ? 'block' : 'hidden'}`}
        style={{
              ...
              <div className="menu-item flex items-center gap-3 p-4">
                <Avatar className="w-12 h-12">
                  ...
                  <AvatarFallback className="bg-gradient-to-br from-blue-500 to-purple-600 text-white font-semibold">
                    {getUserInitials(profile?.username || authUser?.user_metadata?.full_name || '', 1, 'U')}
                  </AvatarFallback>
                </Avatar>
