...
const Header: React.FC = () => {
  ...
  useEffect(() => {
    if (isMenuOpen) {
      ...
      // Aplicar estilos para fijar la posición
      document.body.style.position = 'fixed';
      document.body.style.top = `-${scrollY}px`;
      document.body.style.width = '100%';
      
      // Función para restaurar el scroll al cerrar el menú
      return () => {
  ...
  useEffect(() => {
    const fetchCategorias = async () => {
      try {
        ...
        const flat: ForoCategoria[] = (
          (json?.data || []) as ApiForoCategoria[]
        ).map((c) => ({
          id: c.id,
          nombre: c.nombre,
          slug: c.slug,
  ...
  const closeAllMenus = (e?: React.MouseEvent) => {
    // Si se proporciona un evento, prevenir la propagación
    if (e) {
      e.stopPropagation();
    }
    // Cerrar los menús después de un pequeño retraso para permitir la navegación
    setTimeout(() => {
      setIsMenuOpen(false);
      setIsUserMenuOpen(false);
      setIsAdminMenuOpen(false);
    ...
    <header className="bg-white/95 dark:bg-black/95 backdrop-blur-md fixed top-0 left-0 right-0 z-50 border-b border-gray-200/50 dark:border-gray-800/50 text-gray-900 dark:text-white shadow-sm w-full">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex h-16 items-center justify-between gap-2 md:gap-4">
          ...
          {/* Navegación principal - Solo Desktop - Ocultar en pantallas menores a 1024px */}
          ...
          <div className="flex items-center gap-1 md:gap-3">
            ...
            <div className="hidden lg:flex items-center gap-2">
              ...
              <Link
                href={authUser ? "/foro/crear" : "/login?redirect=/foro/crear"}
              >
                <Button
                  size="sm"
                  variant="outline"
            ...
            <div className="hidden lg:flex items-center gap-4">
              {authUser ? (
                <div className="relative">
                  ...
                  <div
                    ref={userMenuRef}
                    className={`fixed md:absolute right-4 md:right-0 top-16 md:top-auto md:mt-2 z-20 w-64 rounded-xl border shadow-lg bg-white/95 dark:bg-black border-gray-200/50 dark:border-gray-800/50 transition-all duration-200 ease-in-out transform origin-top-right ${
                      isUserMenuOpen
                        ? "scale-100 opacity-100"
                        : "scale-95 opacity-0 pointer-events-none"
                    }`}
      ...
      {isMenuOpen && (
        <div className="lg:hidden">
          ...
          <div
            key="mobile-menu"
            className="fixed top-16 right-0 w-72 max-w-[calc(100%-1rem)] bg-white dark:bg-black rounded-bl-lg border border-gray-200 dark:border-gray-800 shadow-lg z-50 overflow-hidden"
            style={{
                    ...
                    {foroCategorias.map((cat) => {
                      ...
                      return (
                        <div key={cat.id} className="mt-1">
                          ...
                          <div
                            className={`ml-4 mt-1 space-y-1 overflow-hidden transition-all duration-200 ease-out ${
                          ...
                          >
                            {cat.subcategorias?.map((subcat) => (
                              <Link
                                ...
                                className={`block px-4 py-2 rounded-md text-xs ${
                                  currentTheme === "light"
                                    ? "text-gray-700 hover:bg-gray-100"
                                    : "text-gray-300 hover:bg-gray-800"
                                }`}
              ...
              {authUser && (
                ...
                >
                  <div className="flex items-center gap-2">
                    <Link
                      href="/perfil"
                      className={`flex-1 flex items-center gap-2 p-2 rounded-md ${
                        currentTheme === "light"
                          ? "text-gray-700 hover:bg-gray-100"
                          : "text-gray-200 hover:bg-gray-800"
                      }`}
