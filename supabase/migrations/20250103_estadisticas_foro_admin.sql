-- =====================================================
-- MIGRACIÓN: Estadísticas Detalladas para Admin del Foro
-- Fecha: 2025-01-03
...
BEGIN
  SELECT json_build_object(
    'total_hilos', (SELECT COUNT(*) FROM foro_hilos WHERE deleted_at IS NULL),
    'total_comentarios', (SELECT COUNT(*) FROM foro_comentarios WHERE deleted_at IS NULL),
    ...
    'comentarios_hoy', (
      SELECT COUNT(*) FROM foro_comentarios 
      WHERE DATE(created_at) = CURRENT_DATE 
      AND deleted_at IS NULL
    ),
    'hilos_semana', (
...
BEGIN
  ...
  SELECT 
    ...
    (SELECT COUNT(*) FROM foro_comentarios fc WHERE fc.hilo_id = h.id AND fc.deleted_at IS NULL) as comentarios_count,
    COALESCE(h.votos_conteo, 0) as votos_conteo,
    h.created_at,
    -- Fórmula de popularidad: (vistas * 0.1) + (comentarios * 2) + (votos * 5)
    (COALESCE(h.vistas, 0) * 0.1 + 
     (SELECT COUNT(*) FROM foro_comentarios fc WHERE fc.hilo_id = h.id AND fc.deleted_at IS NULL) * 2 + 
...
BEGIN
  ...
  SELECT 
    ...
    c.nivel,
    c.es_activa,
    (SELECT COUNT(*) FROM foro_hilos h WHERE h.categoria_id = c.id AND h.deleted_at IS NULL) as total_hilos,
    (SELECT COUNT(*) FROM foro_comentarios fc 
     INNER JOIN foro_hilos h ON fc.hilo_id = h.id 
     WHERE h.categoria_id = c.id AND fc.deleted_at IS NULL AND h.deleted_at IS NULL) as total_comentarios,
    (SELECT COALESCE(SUM(h.vistas), 0) FROM foro_hilos h WHERE h.categoria_id = c.id AND h.deleted_at IS NULL) as total_vistas,
    (SELECT COUNT(*) FROM foro_hilos h 
     WHERE h.categoria_id = c.id 
     AND h.created_at >= CURRENT_DATE - INTERVAL '7 days'
     AND h.deleted_at IS NULL) as hilos_activos_semana,
...
BEGIN
  ...
  SELECT 
    ...
    (SELECT COUNT(*) FROM foro_comentarios fc 
     WHERE fc.autor_id = p.id 
     AND fc.deleted_at IS NULL
     AND (periodo_dias = 0 OR fc.created_at >= CURRENT_DATE - (periodo_dias || ' days')::INTERVAL)) as total_comentarios,
    ...
    ((SELECT COUNT(*) FROM foro_hilos h 
     ...
     (SELECT COUNT(*) FROM foro_comentarios fc 
      WHERE fc.autor_id = p.id 
      AND fc.deleted_at IS NULL
      AND (periodo_dias = 0 OR fc.created_at >= CURRENT_DATE - (periodo_dias || ' days')::INTERVAL)) * 2 +
     (SELECT COALESCE(SUM(h.votos_conteo), 0) FROM foro_hilos h 
      WHERE h.autor_id = p.id 
      AND h.deleted_at IS NULL
  ...
  WHERE EXISTS (
    ...
    SELECT 1 FROM foro_comentarios fc 
    WHERE fc.autor_id = p.id 
    AND (periodo_dias = 0 OR fc.created_at >= CURRENT_DATE - (periodo_dias || ' days')::INTERVAL)
  )
...
BEGIN
  ...
  SELECT 
    ...
    COALESCE((SELECT COUNT(*) FROM foro_hilos h 
              WHERE DATE(h.created_at) = f.fecha 
              AND h.deleted_at IS NULL), 0) as nuevos_hilos,
    COALESCE((SELECT COUNT(*) FROM foro_comentarios fc 
              WHERE DATE(fc.created_at) = f.fecha 
              AND fc.deleted_at IS NULL), 0) as nuevos_comentarios,
    COALESCE((SELECT SUM(h.vistas) FROM foro_hilos h 
              WHERE DATE(h.created_at) = f.fecha 
              AND h.deleted_at IS NULL), 0) as total_vistas
  FROM fechas f
...
BEGIN
  ...
  SELECT 
    ...
    h.slug as hilo_slug,
    fc.parent_id,
    COALESCE(fc.votos_conteo, 0) as votos_conteo,
    fc.created_at,
    fc.updated_at,
    COALESCE(fc.editado, false) as editado
  FROM foro_comentarios fc
  LEFT JOIN perfiles p ON fc.autor_id = p.id
  LEFT JOIN foro_hilos h ON fc.hilo_id = h.id
...
BEGIN
  ...
  SELECT 
    ...
    LEFT(fc.contenido, 200)::TEXT as contenido,
    p.username as autor_username,
    fc.created_at,
    ('/foro/' || c.slug || '/' || h.slug || '#comentario-' || fc.id)::TEXT as url_relativa
  FROM foro_comentarios fc
  LEFT JOIN perfiles p ON fc.autor_id = p.id
  LEFT JOIN foro_hilos h ON fc.hilo_id = h.id
  LEFT JOIN foro_categorias c ON h.categoria_id = c.id
  WHERE fc.deleted_at IS NULL
    AND fc.contenido ILIKE '%' || termino_busqueda || '%'
...
-- Nota: Los índices se crean sin filtro WHERE deleted_at IS NULL porque esa columna podría no existir
...
CREATE INDEX IF NOT EXISTS idx_foro_comentarios_created_at ON foro_comentarios(created_at DESC);
CREATE INDEX IF NOT EXISTS idx_foro_comentarios_hilo_id ON foro_comentarios(hilo_id);
CREATE INDEX IF NOT EXISTS idx_foro_comentarios_autor_id ON foro_comentarios(autor_id);
