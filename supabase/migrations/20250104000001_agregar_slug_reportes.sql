-- Actualizar la función obtener_reportes_foro para incluir el slug del hilo
...
BEGIN
    ...
    SELECT 
        ...
        CASE 
            WHEN r.tipo_contenido = 'hilo' THEN (SELECT slug FROM public.foro_hilos WHERE id = r.contenido_id)
            WHEN r.tipo_contenido = 'post' THEN (SELECT h.slug FROM public.foro_posts fp JOIN public.foro_hilos h ON fp.hilo_id = h.id WHERE fp.id = r.contenido_id)
            ELSE NULL
        END AS hilo_slug,
        CASE 
            WHEN r.tipo_contenido = 'hilo' THEN r.contenido_id
            WHEN r.tipo_contenido = 'post' THEN (SELECT hilo_id FROM public.foro_posts WHERE id = r.contenido_id)
            ELSE NULL
        END AS hilo_id
