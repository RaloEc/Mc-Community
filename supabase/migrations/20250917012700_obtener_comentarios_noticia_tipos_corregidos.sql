...
RETURNS TABLE (
    ...
    autor_id UUID,
    username VARCHAR, -- Cambiado de TEXT a VARCHAR para coincidir con el tipo de la columna en perfiles
    avatar_url VARCHAR, -- Cambiado de TEXT a VARCHAR para coincidir con el tipo de la columna en perfiles
    color VARCHAR, -- Cambiado de TEXT a VARCHAR para coincidir con el tipo de la columna en perfiles
    es_admin BOOLEAN,
    es_autor BOOLEAN,
    es_propio BOOLEAN,
    respuestas JSONB
...
BEGIN
    ...
    SELECT 
        ...
        p.username::VARCHAR, -- Asegurar que el tipo sea VARCHAR
        p.avatar_url::VARCHAR, -- Asegurar que el tipo sea VARCHAR
        p.color::VARCHAR, -- Asegurar que el tipo sea VARCHAR
        (p.role = 'admin')::BOOLEAN AS es_admin,
        FALSE AS es_autor, -- Por ahora no tenemos esta información
        (cn.usuario_id = v_user_id)::BOOLEAN AS es_propio,
